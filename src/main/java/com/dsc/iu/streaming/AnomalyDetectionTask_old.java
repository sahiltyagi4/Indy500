...
//ADD SPATIAL ANOMALY LOGIC
public class AnomalyDetectionTask_old extends BaseRichSpout implements MqttCallback {
	/**
	 * 
	 */
	...
	private PrintWriter outwrtr;
	private ConcurrentHashMap<String, Queue<Double>> anomalyScoreOuts;
	private Queue<JSONObject> outMessages;
	
	public AnomalyDetectionTask_old(String carnum) {
	...
	public void open(Map conf, TopologyContext context, SpoutOutputCollector collector) {
		...
		for(int i=0; i<metrics.length; i++) {
			...
			startHTMNetwork(metrics[i]);
		}
		
		File f = new File("/scratch_ssd/sahil/sink/sink-"+carnum+".csv");
		try {
			outwrtr = new PrintWriter(f);
		} catch(IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void nextTuple() {
		...
		if(nonblockingqueue.size()>0) {
			...
			//System.out.println("########################## message mqtt:"+record);
			if (record.split(",").length == 6) {
				double speed = Double.parseDouble(record.split(",")[0]);
				double rpm = Double.parseDouble(record.split(",")[1]);
				...
				//System.out.println("@@@@@@@@@@@@@@@ going to put timeOfDay:" + timeOfDay);
...
//					if(recordobj != null && recordobj.containsKey("vehicleSpeedAnomaly") && recordobj.containsKey("engineSpeedAnomaly") 
//							&& recordobj.containsKey("throttleAnomaly")) {
...
//              				&& recordobj.containsKey("throttleAnomaly")) {
//              			
//              			outwrtr.println(carnum + "_" + recordobj.get("timeOfDay") + "," + recordobj.get("vehicleSpeed") + "," +
//              							recordobj.get("engineSpeed") + "," + recordobj.get("throttle") + "," + System.currentTimeMillis());
//              			itr.remove();
//              		}
//				}
                ...
                if(!anomalyScoreOuts.get(metrics[0]).isEmpty() && !anomalyScoreOuts.get(metrics[1]).isEmpty() && !anomalyScoreOuts.get(metrics[2]).isEmpty()) {
                		...
                		outwrtr.println(msgToSend.get("carnum") + "_" + msgToSend.get("timeOfDay") + "," + msgToSend.get(metrics[0]) + "," +
                        			msgToSend.get(metrics[1]) + "," + msgToSend.get(metrics[2]) + "," + msgToSend.get(metrics[0] +"Anomaly") + "," + 
                        			msgToSend.get(metrics[1] + "Anomaly") + "," + msgToSend.get(metrics[2] +"Anomaly") + "," + System.currentTimeMillis());
                }
	...
	private void startHTMNetwork(String metric) {
		String config = "{\"aggregationInfo\": {\"seconds\": 0, \"fields\": [], \"months\": 0, \"days\": 0, \"years\": 0, \"hours\": 0, \"microseconds\": 0, \"weeks\": 0, \"minutes\": 0, \"milliseconds\": 0}, "
...
//               String key = carnum + "_" + timestamp.toString();
                //System.out.println("%%%%%%%%% datetime key value:" + key);
...
//                
//                recordobj.put("carnum", carnum);
//                recordobj.put("timeOfDay", timestamp.toString());
