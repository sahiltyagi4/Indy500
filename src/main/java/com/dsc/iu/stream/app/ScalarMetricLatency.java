...
public class ScalarMetricLatency extends BaseRichBolt {
	...
	public void execute(Tuple arg0) {
		...
		manualpublish.onNext(arg0.getStringByField(getMetric())+","+ arg0.getStringByField("counter"));
	}
	...
	public void prepare(Map arg0, TopologyContext arg1, OutputCollector arg2) {
		...
		Sensor<ObservableSensor<String[]>> sensor = Sensor.create(ObservableSensor::create, SensorParams.create(Keys::obs, 
													new Object[] {"two_metricStreaming", manualpublish }));
		Parameters params = getParams();
		params = params.union(getNetworkLearningEncoderParams());
		Network network = Network.create("two_metric_anomaly_detection", params)
	...
	public void declareOutputFields(OutputFieldsDeclarer arg0) {
		arg0.declare(new Fields("carnum","metric","dataval","score","counter"));
	}
	...
	private static Map<String, Class<? extends Classifier>> getInferredFieldsMap(String field, Class<? extends Classifier> classifier) {
        ...
        inferredFieldsMap.put("counter", classifier);
        
        return inferredFieldsMap;
    }
	
	private Map<String, Map<String, Object>> getNetworkDemoFieldEncodingMap() {
		Map<String, Map<String, Object>> fieldEncodings = setupMap(null, 50, 21, getMin(), getMax(), 0, 0.1, null, Boolean.TRUE, null, 
																getMetric(), "float", "ScalarEncoder");
		fieldEncodings = setupMap(fieldEncodings, 50, 21, Long.MIN_VALUE, Long.MAX_VALUE, 0, 0.1, Boolean.FALSE, Boolean.FALSE, Boolean.FALSE, 
								"counter", "float", "ScalarEncoder");
	...
	private Subscriber<Inference> getSubscriber() {
        return new Subscriber<Inference>() {
            ...
            @Override public void onNext(Inference infer) {
            		...
            		collector.emit(new Values(carnum, getMetric(), String.format("%3.2f", actual_val), infer.getAnomalyScore(), String.valueOf(ctrval)));
//            		System.out.println("***scalar metric out," + getMetric() + "_" + String.valueOf(ctrval) + "_" + carnum + "," 
//            							+ String.format("%3.2f", actual_val) + "," + infer.getAnomalyScore() + "," + emitTs);
