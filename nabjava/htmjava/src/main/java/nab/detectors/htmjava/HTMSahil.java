...
//            }
            ...
            FileOutputStream datalog = new FileOutputStream(new File("/Users/sahiltyagi/Desktop/benchmarks/HTMjava/modifiedHTMparams/dataHTM13_likelihood.csv"));
            BufferedWriter wrtr2 = new BufferedWriter(new OutputStreamWriter(datalog));
            FileOutputStream op = new FileOutputStream(new File("/Users/sahiltyagi/Desktop/benchmarks/HTMjava/modifiedHTMparams/javaNAB13_likelihood.csv"));
            BufferedWriter wrtr = new BufferedWriter(new OutputStreamWriter(op));
            ...
            if (options.has("p")) {
                ...
                if(options.has("o")) {
                    try { op.flush(); op.close(); }catch(Exception ignore) {}
                }
...
//            AnomalyLikelihood likelihood = new AnomalyLikelihood(false, 0, true, 0, 1000);
            AnomalyLikelihood likelihood = new AnomalyLikelihood(false, 375, false, 375, 375);
           // AnomalyLikelihood likelihood = new AnomalyLikelihood(false, 8640, false, 288, 100);
            ...
            network.observe().subscribe((Inference inference) -> {
                ...
                System.out.println("13," + (record + 1) + ",speed," + value + "," + anomaly_likelihood);
                LOGGER.trace("record = {}, score = {}", record, score);
                // Output raw anomaly score
                try {
                		wrtr.write("20," + (record + 1) + ",speed," + value + "," + anomaly_likelihood + "," + System.currentTimeMillis() + "\n");
                } catch(IOException e) {
                		e.printStackTrace();
                }
            }, (error) -> {
...
//                publisher.onNext(line.split(",")[0] + "," + line.split(",")[1]);
                ...
                wrtr2.write(line.split(",")[0].substring(0, line.split(",")[0].length()-4) + "," + line.split(",")[1] + "," + ctr + "," + System.currentTimeMillis() + "\n");
            }
